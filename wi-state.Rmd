...
## Step 0: Clean the text file

```{r}
clean_file <- function(file_path) {
  raw_content <- readBin(file_path, what = "raw", n = file.info(file_path)$size)
  text_content <- rawToChar(raw_content)
  text_content <- gsub("\0", "", text_content)  # Remove null characters
  return(strsplit(text_content, "\n")[[1]])  # Split by lines
}
```
